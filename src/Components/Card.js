const Card = (props) => {
    return (
        <a className="pxp-jobs-card-4 pxp-has-border" id={`job-${props.id}`} data-bs-toggle="tab" data-bs-target={`#job-details-${props.id}`} role="tab" aria-controls={`job-details-${props.id}`} aria-selected="true">
